...
components mount as render elements that own their `Rect` and paint into a
double-buffered character grid **as their artifact response** — non-component
elements define their own artifact response, taken literally. There is no
side-car driver
and no paint delegate: the stage element is the render root *and* the
...
|---|---|---|
| `RenderComponent` | `RenderObjectWidget` | immutable config for a render-bearing node |
| `RenderElement` | `RenderObjectElement` + `RenderObject` (collapsed) | mounted node owning geometry (`rect`) and paint; one type, because a cell grid needs no separate retained render node |
| render element paint-in-rebuild | `RenderObject.paint` via `RenderObjectElement` | `performRebuild` marks needing paint; the same frame pass paints the rect |
| `RenderElement.renderParent` | `RenderObject.parent` | typesetting's own render-tree threading, linked at mount across intervening component elements |
| `RenderParentLink` + mount-time ancestor walk | `RenderObjectElement._findAncestorRenderObjectElement()` / `attachRenderObject` | how a mounting render element finds its enclosing render parent |
| `adoptRenderChild` / `dropRenderChild` | `RenderObject.adoptChild` / `dropChild` | parent-pointer + owner propagation + relayout marking |
| `Stage` / `StageElement` | `RenderView` (+ the root binding) | the root surface; owns dimensions, the byte sink, and the binding |
| `BuildOwner` | `BuildOwner` | dirty set + depth-ordered flush (package `genesis_tree`) |
...
| `markNeedsPaint` → `StageBinding` dirty set | `RenderObject.markNeedsPaint` → `owner!._nodesNeedingPaint.add(this)` | every render element registers directly — the grid is a single surface, so the repaint-boundary walk is unnecessary |

## The vocabulary (v1 — RenderObjects, not a widget library)

| Component | Element | Role |
|---|---|---|
| `Stage(width, height, sink, children, onFrame?)` | `StageElement` | root surface; stacks its children's render elements top-to-bottom full-width; owns the `StageBinding` and frame instrumentation |
| `Box(title, children, accent?)` | `BoxElement` | titled bordered region; stacks its children's render elements as lines inside the border |
| `Text(content)` | `TextElement` | one glyph run / name-value line |
...
// edge) -> scheduleMicrotask -> owner.flush() -> List<Element> rebuilt
// -> flow relayout (if shape changed) -> repaint exactly the dirty render
// elements' rects -> grid.swap() -> AnsiEncoder -> sink
```
...
element (`StageElement.frames`) for tests and ops. Zero-change frames are
...
`Element` exposes no parent pointer, and the tree's component elements do not
thread slots — so the `attachRenderObject` climb is realized with the one
public ancestor walk the tree ships: render containers wrap each child component
in an `InheritedComponent<RenderParentLink>` carrying one identity-stable link to
their element, and a mounting render element resolves
`dependOnInheritedValueOfExactType<RenderParentLink>()` and attaches to the
nearest link. Watch/Stateless/Inherited wrappers — and perception's `Node` —
between two render elements compose transparently, exactly as component
...
elements instead of a paint delegate. Demo (44×12, full-redraw baseline 1297 bytes/frame):
...
cells are confined to the rebuilt render element's rect; zero cells change in
static boxes' rects; the static subtree is never rebuilt (build counter
pinned at 1, element identity preserved); an identical-value event repaints
but diffs to 0 cells / 0 bytes; render elements separated by
...
element identity preserved; and `FrameRecord.rebuilt` is the verbatim
...
components with a Stateless adapter — `Node('ticker', [Field('count', 3), ...])`
