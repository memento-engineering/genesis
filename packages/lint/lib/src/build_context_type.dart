...
/// Whether [type] is a canonical or legacy build context from `genesis_tree`.
///
...
bool isBuildContextType(DartType? type) =>
    _isGenesisInterface(type, const {'BuildContext', 'TreeContext'});

/// Whether [type] is an element or build-context implementation managed by
/// `genesis_tree`, including legacy spellings from older releases.
///
/// Storage owned by one of these types is framework storage, rather than an
/// unmanaged object retaining a tree capability.
bool isTreeManagedType(DartType? type) => _isGenesisInterface(type, const {
  'Element',
  'Branch',
  'BuildContext',
  'TreeContext',
...
/// even when its owning type is not an element or build context.
