...
class UseBuildContextSynchronouslyRule extends AnalysisRule {
  /// The diagnostic reported for an unguarded post-`await` use.
  static const LintCode code = LintCode(
    'use_tree_context_synchronously',
    'Do not use a BuildContext across an async gap without a mounted check.',
    correctionMessage:
        'Check that this BuildContext is mounted after awaiting.',
    uniqueName: 'LintCode.use_tree_context_synchronously',
  );

  /// Creates the rule.
  UseBuildContextSynchronouslyRule()
    : super(
        name: 'use_tree_context_synchronously',
        description: 'Requires mounted checks after BuildContext async gaps.',
      );
  ...
  void registerNodeProcessors(
    ...
    registry.addSimpleIdentifier(
      this,
      _UseBuildContextSynchronouslyVisitor(this),
    );
  }
}

final class _UseBuildContextSynchronouslyVisitor
    extends SimpleAstVisitor<void> {
  _UseBuildContextSynchronouslyVisitor(this.rule);

  final UseBuildContextSynchronouslyRule rule;
  ...
  void visitSimpleIdentifier(SimpleIdentifier node) {
    ...
    if (!isBuildContextType(type)) return;

    final baseElement = _canonicalElement(element);
    if (baseElement == null) return;
    if (_BuildContextAsyncStateTracker(node, baseElement).isUnsafe) {
      rule.reportAtNode(node);
    }
  }
}

final class _BuildContextAsyncStateTracker {
  _BuildContextAsyncStateTracker(this.use, this.baseElement);
...
Element? _mountedBase(Expression expression) {
  ...
  if (target == null || !isBuildContextType(target.staticType)) return null;
...
bool _isMountedProbe(SimpleIdentifier node) {
  for (
    ...
    if (target != null &&
        ...
        target.end >= node.end &&
        isBuildContextType(target.staticType)) {
      return true;
    }
...
/// Legacy name for [UseBuildContextSynchronouslyRule].
@Deprecated('Use UseBuildContextSynchronouslyRule instead.')
typedef UseTreeContextSynchronouslyRule = UseBuildContextSynchronouslyRule;
