...
/// Rejects a `BuildContext` retained in unmanaged durable state.
class NoStoredBuildContextRule extends AnalysisRule {
  /// The diagnostic reported for a retained `BuildContext`.
  static const LintCode code = LintCode(
    'no_stored_tree_context',
    'Do not store a BuildContext in durable state.',
    correctionMessage: 'Use the BuildContext only in its synchronous scope.',
    uniqueName: 'LintCode.no_stored_tree_context',
  );

  /// Creates the rule.
  NoStoredBuildContextRule()
    : super(
        name: 'no_stored_tree_context',
        description: 'Rejects BuildContext capabilities retained in state.',
      );
  ...
  void registerNodeProcessors(
    ...
    final visitor = _NoStoredBuildContextVisitor(this);
    registry
...
final class _NoStoredBuildContextVisitor extends SimpleAstVisitor<void> {
  _NoStoredBuildContextVisitor(this.rule);

  final NoStoredBuildContextRule rule;
  ...
  void visitAssignmentExpression(AssignmentExpression node) {
    ...
    if (isBuildContextType(value.staticType)) rule.reportAtNode(value);
  }
  ...
  void visitFieldFormalParameter(FieldFormalParameter node) {
    if (_isManagedOwner(node)) return;
    if (isBuildContextType(node.declaredFragment?.element.type)) {
  ...
  void visitMethodInvocation(MethodInvocation node) {
    ...
    for (final argument in storedArguments) {
      ...
      if (storesIterable
          ? _isIterableOfBuildContext(argument.staticType)
          : isBuildContextType(argument.staticType)) {
  ...
  void visitVariableDeclaration(VariableDeclaration node) {
    ...
    if (value != null && isBuildContextType(value.staticType)) {
  ...
  void _reportCollectionElement(CollectionElement element) {
    switch (element) {
      case Expression expression:
        if (isBuildContextType(expression.staticType)) {
          rule.reportAtNode(expression);
        }
      case MapLiteralEntry entry:
        if (isBuildContextType(entry.key.staticType)) {
          rule.reportAtNode(entry.key);
        }
        if (isBuildContextType(entry.value.staticType)) {
          rule.reportAtNode(entry.value);
        }
      case SpreadElement spread:
        if (_isIterableOfBuildContext(spread.expression.staticType)) {
      ...
      case NullAwareElement nullAware:
        if (isBuildContextType(nullAware.value.staticType)) {
...
final class _CapturedContextVisitor extends RecursiveAstVisitor<void> {
  ...
  void visitSimpleIdentifier(SimpleIdentifier node) {
    ...
    if (isBuildContextType(variable.type)) rule.reportAtNode(node);
  }
}
...
bool _isIterableOfBuildContext(DartType? type) {
  ...
  for (final candidate in <InterfaceType>[
    ...
    if (candidate.element.name == 'Iterable' &&
        ...
        isBuildContextType(candidate.typeArguments.first)) {
      return true;
    }
  }
  return false;
...
/// Legacy name for [NoStoredBuildContextRule].
@Deprecated('Use NoStoredBuildContextRule instead.')
typedef NoStoredTreeContextRule = NoStoredBuildContextRule;
