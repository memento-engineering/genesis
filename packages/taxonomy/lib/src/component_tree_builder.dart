/// Flat keyed components -> `Component` tree, parameterized over the registry.
///
...
class ComponentInstance {
  ...
  /// Stable component id — becomes the `Component` key (the reconciliation
...
/// Builds a `Component` tree from the flat [components] list through [registry].
///
/// Component ids become `Component` keys, which is what lets keyed reconciliation
...
Component buildComponentTree(
  ...
  Component build(String id, String? parentId) {
    ...
    visiting.removeLast();
    // Component id becomes the Component key — the reconciliation identity. The
...
/// Legacy spelling for [buildComponentTree].
@Deprecated('Use buildComponentTree instead.')
Component buildSeedTree(
  ComponentRegistry registry,
  List<ComponentInstance> components, {
  String rootId = 'root',
}) => buildComponentTree(registry, components, rootId: rootId);
