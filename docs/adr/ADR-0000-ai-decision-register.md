...
**Affects:** `packages/tree/lib/src/branch.dart`; A14 inclusion-rule wording; ADR-0001 Decision 4 gains an "unless identical" clause at promotion; perception inherits (conformance suite is the gate). Ten gating tests proposed in the analysis doc. **Status:** pending — awaiting Nico (decision bead `genesis-ak7` stays open).
...
**Affects:** ADR-0002 at promotion; `genesis_dialogue` consumes `buildSeedTree` + the envelope shape. **Status:** pending.
...
**Affects:** ADR-0004 at promotion; first expression-row consumer of the composition layer (one of the two consumers the A11 freeze rule needs). **Status:** superseded by A23/A24 (2026-06-12) — `Typesetter`/`PaintDelegate`/`Region`/`subtreeContains` deleted in the render-branch rework; the cell core, economy record, and the barrel-does-not-re-export-tree call survive.
...
**Affects:** `genesis_taxonomy` API (renamed, gates green); future packages; genesis `CLAUDE.md` conventions (rule added). **Status:** decided (Nico 2026-06-12).
...
**Affects:** `genesis_typesetting` (rebuilt; cell core/encoder/economy tests survive); A20 (superseded in part); ADR-0004 wording at promotion; `genesis_expression`'s backend follows the same render-seed pattern. **Status:** decided (Nico 2026-06-12) — rework bead `genesis-0uv`; **built + landed** commit `27b0802` (A24 records the surface).
...
- **A18 interaction (for Nico's pending call):** the A9 no-fast-path cascade makes a `Box` update repaint its `Text` children every time — locality holds at the box rect and the buffer dedups, but landing A18's `identical()` skip would shrink the dirty-paint set further. Economy through the new architecture reproduced the spike record exactly (268 update bytes vs ~12,970 full redraws, ~48×).
**Affects:** ADR-0004 at promotion; the `genesis_expression` backend pattern; A18's cost/benefit ledger. **Status:** pending.
