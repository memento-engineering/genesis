...
- **Catalog format:** `{catalog: {name, version, description?}, types: {...}}`; core type-level keys are exactly `description`/`container`/`props`/`dart`; **every other type-level key is plugin vocabulary** (seam 1); unknown keys anywhere else → `CatalogFormatException`.
...
- **Plugin seam:** `CatalogPlugin{name, typeKeys, parseTypeValue, augmentToolSchemaVariant}`; ALL unclaimed type-level keys across the catalog throw once as `UnhandledCatalogKeysException` (loud, lists keys + registered plugins). **Actions ride the seam as the proof** (`ActionsCatalogPlugin` → `CatalogType.actions` + `x-actions` in the tool schema — `genesis_consent` consumes later).
...
- **build_runner:** `*.catalog.json` → sibling `.g.dart` + `.g.json`, `build_to: source` with an in-sync test as the guard; the shipped builder runs default plugins only — custom plugins wrap `generateFromCatalog`.
...
**Affects:** ADR-0004 at promotion; first expression-row consumer of the composition layer (one of the two consumers the A11 freeze rule needs). **Status:** pending.
