...
**Affects:** ADR-0004 at promotion; first expression-row consumer of the composition layer (one of the two consumers the A11 freeze rule needs). **Status:** pending. *(Fixtures corrected 2026-06-12 per A22.)*
...
**Affects:** `genesis_typesetting` (rebuilt; cell core/encoder/economy tests survive); A20 (superseded in part); ADR-0004 wording at promotion; `genesis_expression`'s backend follows the same render-seed pattern. **Status:** decided (Nico 2026-06-12) — rework bead `genesis-0uv`.
