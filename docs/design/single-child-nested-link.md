# Single-child links, the `nested` way — `SingleChildComponent` as a marker + `Nest` as a link
...
Decisions 3–5 (Element purity), register A16/A21.
...
1. **`Nest` should itself be a `SingleChildComponent`** so a `Nest` can be a link inside another
   `Nest` (chains compose / flatten).
2. The single-child stateless/stateful components should **build on the `StatelessComponent` /
   `StatefulComponent` / `State` primitives**, not re-clone them.

The old `single_child.dart` did neither. `SingleChildComponent` was a concrete base carrying a
`child` field, and the slot-in mechanism was a hard **downcast** inside the chain carrier:

```dart
// old _Link.createElement()
node is SingleChildStatefulComponent ? _StatefulLinkBranch(this) : _StatelessLinkBranch(this);
// old _StatelessLinkBranch.build()
(link.node as SingleChildStatelessComponent).buildWithChild(context, link.next);
```

Anything that isn't one of those two concrete component subclasses fails the cast — so
`Nest` (or `InheritedComponent`) could never slot in, no matter what interface it declared. And
the stateful path was a **parallel clone**: `SingleChildState` re-implemented `State`'s whole
lifecycle, and `_StatefulSingleChildBranch` re-derived `StatefulElement`'s
...
|---|---|
| `SingleChildWidget implements Widget` (marker) | `abstract interface class SingleChildComponent implements Component` |
| `SingleChildWidgetElementMixin on Element` | `mixin SingleChildElementMixin on Element` |
| `SingleChildStatelessWidget extends StatelessWidget` | `SingleChildStatelessComponent extends StatelessComponent` |
| `SingleChildStatefulWidget extends StatefulWidget` | `SingleChildStatefulComponent extends StatefulComponent` |
| `SingleChildState extends State` | `SingleChildState extends State` |
| `StatelessElement with …Mixin` | `SingleChildStatelessElement extends StatelessElement with SingleChildElementMixin` |
| `StatefulElement with …Mixin` | `SingleChildStatefulElement extends StatefulElement with SingleChildElementMixin` |
| `_NestedHook` / `_NestedHookElement` (carries `injectedChild`) | `_NestHook` / `_NestHookBranch` |
| `Nested extends StatelessWidget implements SingleChildWidget` | `Nest extends Component implements SingleChildComponent` |
...
**Result:** the parallel `State` clone and the bespoke stateful link element are deleted;
single-child stateful now inherits `StatefulElement`'s full lifecycle
...
The hook topology means the node's own element reads its downstream from a *separate* hook
...
## 4. Element purity (ADR-0001 D3 / register A31)

`SingleChildElementMixin` is a composition-layer mixin *on* `Element`; it adds no
render/gesture/timer/listener machinery to `Element` core — it only reroutes which downstream
a build embeds. `Element` itself is untouched.

## 5. Compatibility / blast radius

- **`SingleChildComponent`: concrete base → interface.** No production code subclasses it (only
  `tree`'s own tests, which extend the concrete `SingleChild{Stateless,Stateful}Component` — those
  stay `abstract class` you extend). `perception`/`typesetting` use `InheritedComponent` and the
...
- **`InheritedComponent` as a link.** The same seam now admits it (an `InheritedElement with
  SingleChildElementMixin` variant — `nested`'s `SingleChildInheritedElementMixin`), which would
  let a provider sit directly in a `Nest` without a wrapper. Left out to keep this reviewable;
  it also carries a `child` required→nullable change on `InheritedComponent` and a `perception`
  (`InheritedPerception`) ripple.
- **Extra element per link.** The hook topology adds one hook element per link (as `nested`
