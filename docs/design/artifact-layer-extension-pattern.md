...
## 1. Why the spine stops at `Component` → `Element`
...
> **`Element` core is artifact-agnostic** — identity, lifecycle, keyed reconcile, dirtiness,
> and **one abstract rebuild hook** (the `performRebuild` analog). It carries **no build
> contract**.

and its Element-purity amendment, folded into the same Decision:

> `Element` stays exactly **identity + keyed reconciliation + dirtiness**, plus the one
...
|---|---|---|---|
| **Artifacts** | `RenderObject` | `RenderElement` — the element *is* the artifact (owns `rect`, paints cells) | the mounted element itself — `NodeElement` / `FieldElement` |
| **Owner** | `PipelineOwner` | `StageBinding` — the dirty-paint set and the frame pass | none; the harvest is pulled, so `PerceptionOwner` only renames `BuildOwner` |
| **Protocol** | layout / paint / hit-test | `flowHeight` → `layout(Rect)` → `paint(CellGrid)` | `serializePerceptionFragment(Element)` — a read over the live tree |
| **Affordance scopes** | `Theme` / `MediaQuery` | `InheritedComponent<RenderParentLink>` — render-parent threading | `InheritedPerception<T>` — ambient measurement values |

Note the asymmetry. Perception defines two of the four pieces trivially: its artifacts *are*
its elements, and it needs no second scheduler. That is the point of unbundling — a consumer
...
An artifact is the persistent, live thing a mounted element owns: a painted region, a
harvestable datum, a spawned process, a held lease. The question that finds it is **"what
does this element own that outlives a single rebuild?"**

Two shapes are both correct:

- **The element *is* the artifact.** `RenderElement` collapses Flutter's `RenderObjectElement`
  ...
  there is no separate retained render node to keep: the element *is* the render object."
  Cheapest; use it when the artifact's lifetime is exactly the element's.
- **The element *holds* the artifact.** The element creates a separate object at mount and
  disposes it at unmount. Use it when the artifact must outlive, or be handed across, element
  identity — the_grid's `Allocation` is this shape (§5).

Either way: mount creates, unmount disposes. `Element.mount` and `Element.unmount`
(`packages/tree/lib/src/element.dart`) are the only lifecycle hooks the spine offers, and they
are enough.

### 2.2 Owner — the pass scheduler beside `BuildOwner`

`BuildOwner` (`packages/tree/lib/src/build_owner.dart`) schedules exactly one pass: the
**build** pass. It holds the dirty set, fires `onNeedsFlush` on the empty→non-empty edge, and
`flush()` drains depth-ordered and **returns the elements it rebuilt** — ADR-0001 Decision 5:
...
2. **`onNeedsFlush` is single-listener.** The binding asserts-and-claims it at mount
   ...
   owners on one `BuildOwner` is not a supported shape today; it would need a multi-listener
   edge on `BuildOwner`, a request the tree has deliberately not been asked for.
...
The protocol is what the pass *does* to the artifacts. `Element` gives you exactly one hook to
respond to a config change — `performRebuild()` — and ADR-0001 Decision 4 fixes when it
fires: a keyed in-place update "**invokes the Element's rebuild hook** — *unless the new component
...
Perception's protocol is a **read**: `serializePerceptionFragment(Element)`
...
- **Traverse with `visitChildren`, not a private child list.** It is shallow, tree-ordered,
  ...
  that way, which is what makes component elements between two render elements transparent:
  `RenderElement.renderChildren` descends until it hits the nearest `RenderElement`.
- **Keep the pass out of `Element`.** Both examples put every scheduling primitive
...
An affordance scope is a value a element reads from its ancestors instead of being handed it
explicitly: Flutter's `Theme`, `MediaQuery`. In genesis it is `InheritedComponent<T>` +
`dependOnInheritedValueOfExactType<T>()` — the one Element-bloat category the spine
...
dependent set allocated lazily so a element with no dependencies pays nothing.
...
```dart
// packages/typesetting/lib/src/render_element.dart — the container side
@protected
...
- **The wrapper's key is namespaced** (`_RenderScopeKey`), never the child's own key.
  Reusing the bare child key would mint a second element answering to that key, and any
...
the context handle — never make the element its own context.** ADR-0001 Decision 2
(`docs/decisions/2026-06-11-adr-0001-foundations.md`, "The tree spine: `Component` → `Element`; `BuildContext` is a
separate handle") is the constraint:

> **The fork — shed Flutter's Element≡BuildContext "original sin":** `Element` does **not**
> implement `BuildContext`. The context is a *distinct capability handle* passed to `build()`,
...
after unmount; a element-as-context would silently act on a dead node. So the sanctioned move
is *wrapping*: `PerceptionContext implements BuildContext` over a private handle that
...
capability extension of `BuildContext` — the domain layers budget/harvest capabilities onto
...
|---|---|---|
| Artifacts | `RenderElement` and its subclasses `StageElement` / `BoxElement` / `TextElement` — each owns a `Rect` and paints cells | `packages/typesetting/lib/src/render_element.dart` |
...
| Protocol | `flowHeight` → `layout(Rect)` → `paint(CellGrid)`, drained parent-first by `paintSubtree` | `packages/typesetting/lib/src/render_element.dart` |
| Affordance scope | `InheritedComponent<RenderParentLink>`, provided by `renderScopeFor` and resolved by `attachRenderParent` | `packages/typesetting/lib/src/render_element.dart` |

The mount path, end to end:

1. `owner.mountRoot(Stage(...))` mounts a `StageElement`, which creates the one `StageBinding`
   and claims `BuildOwner.onNeedsFlush`. Frame 0 paints synchronously at mount, so mounting
   the stage *is* the entry shape.
2. Every render container wraps each child component in `renderScopeFor(child)` before reconciling
   it, so the child mounts inside that container's render scope.
3. `RenderElement.mount` calls `attachRenderParent()`, which resolves the nearest
   `RenderParentLink` and lets that element `adoptRenderChild(this)` — setting the parent
...
beside the element, and downward adjacency (`renderChildren`) is derived from `visitChildren`
...
| Owner | none beyond the spine — `PerceptionOwner extends BuildOwner`, renaming `flush()` → `flushHarvest()` and `onNeedsFlush` → `onNeedsHarvest` | `packages/perception/lib/src/perception_owner.dart` |
| Protocol | harvest-on-read: `serializePerceptionFragment(Element)` folds the live subtree into a JSON map | `packages/perception/lib/src/serialize.dart` |
| Affordance scope | `InheritedPerception<T>` — the domain face of `InheritedComponent<T>` | `packages/perception/lib/src/inherited_perception.dart` |
...
- **Composition stays tree-owned.** `PerceptionElement extends Element` is reserved for
  *artifact* elements. The stateless / stateful / inherited perception classes subclass the
  tree's composition elements and only upgrade the handle — Flutter's `ComponentElement` vs
...
- **Artifacts:** an `Allocation` — a persistent, addressable managed object holding one live
  effect. Held *by* a element rather than equal to it, because the effect must survive
...
- **Affordance scopes:** scope elements (`SessionScope`, `SubstationScope`) that create
...
genesis the clean substrate. The symptom it cured was live effects smeared into the element
...
From `Element`'s perspective the artifact layer is not a tree, or anything else in particular.
Artifacts can hang off elements individually, with every relationship carried by the element
...
relationship the element tree cannot carry — a parent that must enumerate, meter, or re-parent
...
1. **Putting the pass on `Element`.** Timers, listeners, post-frame-style callbacks, and
...
2. **A second dirty mechanism.** Your pass consumes `BuildOwner.flush()`'s return value; do
...
3. **Making the element its own context.** That re-commits Flutter's Element≡BuildContext
   sin, which ADR-0001 Decision 2 sheds: "`Element` does **not** implement `BuildContext`. The
   context is a *distinct capability handle* passed to `build()`, never the mounted node
   itself." Layer a handle over `BuildContext` instead (§2.4); `PerceptionContext` is the
...
4. **Reusing a child's key on a scope wrapper.** Two elements then answer to one key, and
   every key-based lookup finds both. Namespace the wrapper's key, as `_RenderScopeKey` does.
5. **Reaching into a element's `State` from the artifact layer.** `StatefulElement.state` is
   `@protected` — subclass-only, not public API (ADR-0001 Decision 3). Expose a narrow seam
   interface on the element and test `element is YourSeam`, the way the action substrate does.

## 8. Checklist for a new consumer

1. **What does a element own that outlives a rebuild?** That is your artifact set (§2.1). If
   the honest answer is "nothing", you do not need an artifact layer.
2. **Is the artifact the element, or held by it?** Equal lifetime → be the element; must
   survive re-adoption → hold it.
3. **Push or pull?** A pushed pass needs an owner over `BuildOwner.flush()` (§2.2); a pulled
...
5. **What must a element read from its ancestors?** Ambient values → `InheritedComponent<T>`;
   domain capabilities → wrap `BuildContext`, never subclass a element into one (§2.4).
6. **Can every relationship ride the element tree?** If yes, retain no links. If no, name the
   relationship before adding one (§6).
7. **Does anything you added belong on `Element`?** If it is a callback the framework calls
